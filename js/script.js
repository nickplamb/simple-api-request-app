...
let triviaSession = (function () {
  ...
  function addListItem(question, questionIndex, numOfQuestions) {
    ...
    let cardRow = $('<div></div').addClass(
      'row justify-content-center card-container__row'
    );
    let card = $('<div></div>').addClass(
      'card-container__card col-11 col-sm-10 col-md-8 col-lg-7 col-xl-5 my-3'
    );
    let questionNumberClass = 'question-' + (questionIndex + 1);
    let cardInner = $('<div></div>').addClass(
      'card card-inner bg-light ' + questionNumberClass
    );
    let cardFront = $('<div></div>').addClass('card-front card-body bg-light');
    let cardHeader = $('<div></div>').addClass('card-header bg-warning mt-3');
    let cardHeaderTitle = $('<h4></h4>')
      .addClass('card-title text-center')
      .text(`Question ${questionIndex + 1}:`);
    let cardHeaderText = $('<p></p>')
      .addClass('card-text')
      .text(question.question);
    let answerList = $('<div></div>')
      .addClass('mt-4 answer-btns')
      .attr('id', 'q-answers-' + (questionIndex + 1));
    let cardBack = $('<div></div>').addClass(
      'card-back card-body bg-light ' + questionNumberClass
    );
    let cardBackResult = $('<div></div>').addClass('mt-4');
    let cardBackAnswer = $('<p></p>').addClass(
      'card-text card-result ' + questionNumberClass
    );
    let cardBackImg = $('<img>').addClass(questionNumberClass + ' result-img');

    // Attach event handler to the answerList element as a delegate.
    answerList.on('click', 'button', (e) =>
      answerHandler(e, question, numOfQuestions, questionNumberClass)
    );
  ...
  function answerHandler(e, question, numOfQuestions, questionNumberClass) {
    ...
    if (selectedAnswer === question.correct_answer) {
      score++;
      $(`.card-result.${questionNumberClass}`).text(
        `The correct answer is: ${question.correct_answer}`
      );
      $(`img.${questionNumberClass}`).attr({
